...
[[https://github.com/scala/scala/pull/5617][=-Xstrict-patmat-analysis=]] flag has been proposed as a language
...
It is unfortunate that this was not the default when the language
...
[[https://tools.ietf.org/html/rfc6749][OAuth2]] for authentication. Although there are many permutations of
...
#+BEGIN_INFO
In these payload examples, =CAPITAL_SYMBOLS= are used as placeholders
for variables, and escaped newlines have been added for clarity.
#+END_INFO
...
=Uri= is a typed ADT for URL requests and is from [[https://github.com/Spinoco/fs2-http][fs2-http]]:
...
Apart from being insane, and *very very* slow, these core JVM methods
can throw I/O exceptions (are not /pure/), and can change depending on
your network configuration (are not /deterministic/).
...
[[https://github.com/circe/circe][circe]] gives us an ADT for the JSON encoding and a typeclass to perform
...
#+BEGIN_WARNING
=java.math.BigDecimal= is not a safe object to include in a wire
protocol format. It is possible to construct valid numerical values
that will exception when parsed and hang the =Thread= forever if
converted into =java.math.BigInteger=.

Travis Brown, maintainer of Circe, has [[https://github.com/circe/circe/blob/master/modules/core/shared/src/main/scala/io/circe/JsonNumber.scala][gone to great lengths]] to
protect us all. If you want to have similarly safe numbers in your
wire protocols, either use =JsonNumber= or settle for lossy =Double=.

We give you /The Infinity Bomb/: `"1e2147483648"`. To freeze your JVM,
...
***** FIXME FIXME FIXME this is as far as I got
