...
If there is an implicit =Monad[T]= available for =T= (i.e. =T= is
monadic as discussed in the introduction) then cats lets us create a
=T= from a value with =.pure[T]=.

There is a =Monad[Future]= and =.pure[Future]= just calls
=Future.successful= which - apart from being slightly shorter to
type - is good practice because it is a general concept.
