...
#+BEGIN_ASIDE
We're breaking more Chapter 4 rules about what can live on a data
type: it is necessary to use a =lazy val= inside =Later= to get the
required semantics. It would not be possible to do this in a
typeclass, because it requires managing state.

Note shown are two more children of =Eval=: =Call= and =Compute=,
which are used to internally trampoline long chains of =map= and
=flatMap=. =Eval= should therefore not be pattern-matched, despite
being a sealed coproduct.
#+END_ASIDE
