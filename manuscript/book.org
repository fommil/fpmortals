...
Norris for the [[https://github.com/tpolecat/cats-infographic][Cats Infographic]].
...
Firstly, it is necessary to use a =lazy val= inside =Later= to get the
required semantics and it would not be possible to do this in an
extension class because the cached =value= would be thrown away on
each method call, thus defeating the point. But that only explains the
=lazy val=, not the methods on =Eval= or the other implementations.

Secondly, there are a lot more methods than just =value= and =memoize=
on =Eval=. There is also =map= and =flatMap=. The reason they live on
the ADT and not in an instance of =Monad= is because it is slightly
more efficient for the compiler to find these methods instead of
looking for =Monad.ops._=, and there is a lower memory cost to pay at
...
And while we're being honest, the actual implementation of =Later= is
slightly more involved, allowing for the =f= to be garbage collected,
and there are another two children of =Eval=, called =Call= and
=Compute=, which are internally used to trampoline long chains of
=map= and =flatMap=.
#+END_ASIDE
