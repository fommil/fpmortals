...
that works as a =case= in a pattern match. Be careful that you don't
miss any cases or you'll get a runtime exception (a /totality/
failure):
...
generating a call to =withFilter=.[fn:filter: Older versions of scala
...
How many times have you seen a function that takes =Option= parameters
but actually requires them all to exist? I hate seeing this:
...
Although this is better than throwing an exception, it is clunky and
bad style. If a function really does require every input then it
should make its requirement explicit, pushing the responsibility for
