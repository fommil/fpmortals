...
#+BEGIN_ASIDE
*Exhaustivity caveats*

We add =-Xfatal-warnings= to our compiler flags and are forced to
update any code that matches over a =sealed trait= when somebody adds
an extra implementation.

Although the scala compiler will perform exhaustivity checks when
matching on sealed traits, e.g.
...
the compiler will not perform exhaustivity checking if there are
guards, e.g.
...
Note that the implementations of =Foo= are not the only concern here:
this will fail at runtime if we pass a =Bar(false)=.

Guards should not be used when matching on a =sealed trait=, and when
used on a =case class= should always include a =case _ =>= catch-all
...
bytecode representation of the data. For example, [[https://github.com/fommil/stalagmite][stalagmite]] can pack
