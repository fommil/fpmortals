...
#+BEGIN_ASIDE
Scalaz is a principled Functional Programming library that has evolved
over a decade of industry use.

Cats is an experiment to rewrite scalaz. Cats is a member of the
[[http://typelevel.org/about.html][Typelevel]] community of projects and people organised around functional
programming in Scala. Typelevel is a political organisation with
moderators who enforce a Code of Conduct to ensure that their spaces
are welcoming, inclusive and safe.

Although this book uses scalaz, a variant using cats will be created
(subject to demand). In the meantime, you can obtain a gratis copy of
[[http://underscore.io/books/advanced-scala/][Advanced Scala with Cats]] by Underscore Consultants. At this point, the
cats API is mostly a clone of scalaz, so this book is relevant for
either library.
#+END_ASIDE
...
# *** TODO https://twitter.com/ValentinKasas/status/879414703340081156
# *** TODO https://github.com/vil1/single_page_fp_book
...
#+BEGIN_ASIDE
It is common to use =|+|= instead of =mappend=, known as the TIE
...
1. =strengthL= pairs the result of an =F[B]= with a constant =A= on the left.
1. =strengthR= pairs the result of an =F[A]= with a constant =B= on the right.
1. =lift= takes a =A => B= and returns a =F[A] => F[B]=, or in
