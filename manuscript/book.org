...
Many developers are surprised that when they start a =Future= in a
=for= comprehension, their calculations do not run in parallel:
...
The is because the =flatMap= spawning =anotherExpensiveCalc= is only
