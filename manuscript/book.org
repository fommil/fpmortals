...
a stack safe =foldRight=, [[https://github.com/scalaz/scalaz/issues/1447][even within scalaz]].
...
#+BEGIN_ASIDE
We've seen the =NonEmptyList= in previous chapters despite have fully
explored its feature set. For the sake of brevity we use a type alias
=Nel= in place of =NonEmptyList=.
