...
type constructor. Let's define a type alias to be equal to its
parameter:
...
Type aliases don't define a new types, they just use substitution to
let us have convenient syntax. Before proceeding, convince yourself
that =Id[Int]= is the same thing as =Int=, by substituting =Int= into
=T=. But =Id= is a valid type constructor, so we can use =Id= in an
