...
If there is an implicit =Monad[T[_]]= for =T= (i.e. =T= is monadic)
then cats lets us create a =T[V]= from a value =v:V= by calling
=v.pure[T]=.

Cats provides =Monad[Future]= and =.pure[Future]= simply calling
=Future.successful=. Apart from =pure= being slightly shorter to type,
it is good practice to use it because it is a general concept that
works for all monadic contexts.
