...
whatever goes here must take a type parameter, and we can use that =C=
in our method signatures. For example:

#+BEGIN_SRC scala
trait Foo[C[_]] {
  def wrap(s: Int): C[Int]
}
#+END_SRC

The /kind/ of a type constructor is the number of type parameters that
it takes to produce a type. For example =List[E]= takes one type
parameter and produces a type, having kind

#+BEGIN_SRC
  * -> *
#+END_SRC

Whereas =Map[K, V]= takes two type parameters and has kind

#+BEGIN_SRC
  * -> * -> *
#+END_SRC

The reason why we use asterisks and don't just say "=List= has kind 2"
or "=Map= has kind 3" is when we are dealing with higher kinded types,
such as =Foo=, which takes a type constructor (denoted with brackets)
and has kind
...
We can implement =Foo= with =List= or =Option=, see how they are in
the return types:

#+BEGIN_SRC scala
object OptionFoo extends Foo[Option] {
  def wrap(s: Int): Option[Int] = Some(s)
}
object ListFoo extends Foo[List] {
  def wrap(s: Int): List[Int] = List(s)
}
#+END_SRC

Scala also allows us to use a =*->*->*= constructor where a =*->*=
should go, which means we can implement =Foo= for something like
=Either[String, _]= (which only has one hole). Unfortunately the
syntax is a bit clunky and we have to create a type alias

#+BEGIN_SRC scala
type EitherString[T] = Either[String, T]
object EitherFoo extends Foo[EitherString] {
 def wrap(s: Int): Either[String, Int] = Right(s)
}
#+END_SRC

We can use a type alias in the other direction too, to remove a layer.
Using type substitution on the =Id= type alias feels like a trick:

#+BEGIN_SRC scala
type Id[T] = T
#+END_SRC

Convince yourself that =Id[Int]= is just =Int= before proceeding, by
substituting =Int= into =T=.
