...
En este capítulo exploraremos los tipos de datos en Scalaz que son similares a
colecciones, así como los tipos de datos que aumentan Scala, como lenguaje, con
...
W> Las estimaciones de rendimiento (incluyendo las que se hacen en este
W> capítulo) deberían ser tomadas con cierto criterio. El diseño moderno de
W> procesadores, la disposición secuencial de memoria, y la recolección de
W> memoria pueden invalidar el razonamiento intuitivo basado en conteo de
W> operaciones.
W>
W> Una realidad dura de la computación moderna, es que las pruebas de
W> rendimiento empíricas, para una tarea específica, pueden causar asombro y
W> sorpresa: por ejemplo, la búsqueda en una `List` es con frecuencia más rápida
W> que en un `Vector`. Use una herramienta como
W> [JMH](http://openjdk.java.net/projects/code-tools/jmh/) cuando realice
W> pruebas de rendimiento.

Todas las colecciones presentadas aquí son *persistentes*: si agregamos o
removemos un elemento todavía podemos usar la versión anterior. Compartir
estructuralmente es esencial para el rendimiento de las estructuras de datos
persistentes, de otra manera la colección entera se reconstruye con cada
operación.

A diferencia de las colecciones de Java y Scala, no hay jerarquía de datos en
Scalaz: estas colecciones son mucho más simples de entender. La funcionalidad
polimórfica se proporciona por instancias opmitizadas de typeclases que
estudiamos en el capítulo anterior. Esto simplifica cambiar implementaciones por
razones de rendimiento, y proporcionar la propia.
...
Desafortunadamente debemos ser cuidadosos cuando construimos tipos de datos
invariantes debido a que los cálculos LUB se realizan sobre los parámetros:
...
compilador](https://issues.scala-lang.org/browse/SI-2509). Considere la
...
La resolución implícita hace un intercambio de su definición de "más específico"
para los tipos contravariantes, logrando que sean inútiles para los typeclases o
para cualquier cosa que requiera funcionalidad polimórfica. Este comportamiento
es fijo en Dotty.
...
A> El símbolo Liskov toma su nombre en honor a Barbara Liskov de la liga de la
A> fama del *principio de substitución de Liskov*, uno de los fundamentos de la
...
A> Si el cálculo de un `lazy val` lanza una excepción, se intenta de nuevo cada
A> vez que se accesa. Debido a que las excepciones pueden romper la
A> transparencia referencial, limitaremos nuestra discusión de los cómputos
A> `lazy val` a aquellos que no lanzan excepciones.
...
A> `Lazy` (con una letra mayúscula `L`) con frecuencia se usa en las librerías
A> de Scala para los tipos de datos con semántica *por nombre*: un mal nombre
A> que ha perdurado.
A>
A> De manera más general, todos somos muy flojos con respecto a cómo hablamos
A> con respecto a la pereza: puede ser bueno buscar clarificación sobre el tipo
A> de pereza que se está discutiendo. O tal vez no, debido a... la flojera.
...
A> el costo extra de rescribir usando este estilo, no sea mayor que el ahorro.
A> No hay beneficio extra a menos que exista la posibilidad de **no** evaluar.
A> El código de alto rendimiento que se ejecuta en un ciclo corto y que siempre
A> se evalúa sufrirá.
...
Algunas veces tenemos dos tipos que en realidad son equivalentes (la misma
cosa), lo que ocasiona problemas de compatibiliad debido a que el compilador no
sabe lo que nosotros sí sabemos. Esto pasa típicamente cuando usamos código de
terceros que tiene algo que ya tenemos.
...
Con frecuencia se usa para representar una cosa que puede o no estar presente
sin proporcionar ninguna información adicional de porqué falta información.
...
A> hizo en la Lección 4 para usar un `object` o una clase implícita. Este es
A> algo común en Scalaz y la razón es grandemente histórica.
A>
...
A> - Existen casos especiales donde el compilador fallará en inferir los tipos y
A>     no será capaz de encontrar los métodos de extensión.
A> - la librería estándar define instancias de clases implícitas que agregan
A>     métodos a todos los valores, con nombres de métodos que entran en
A>     conflicto. `+` es el ejemplo más prominente, que convierte todo en una
...
A> Una de las cosas más lentas en la JVM es la creación de una excepción, debido
A> a la cantidad de recursos requeridos en la construcción del stacktrace. Es
...
A> embargo, el stacktrace dentro de la excepción depende de la cadena de
A> llamadas, dando un valor diferente dependiendo de quién haga las llamadas,
A> por lo tanto violando la transparencia referencial. Sin embargo, lanzar una
A> excepción no es puro debido a que significa que la función no es *Total*.
...
Con nuestra interpretación de nuestro programa, podemos realizar aserciones
sobre los métodos que son invocados:
...
esencial recordar todo lo que se presenta aquí: la meta es ganar entendimiento a
un nivel de abstracción alto de cómo funciona cada estructura de datos.

Debido a que los tipos de datos de todas las colecciones nos proporcionan más o
menos la misma lista de instancias de typeclases, debemos evitar repetir la
lista, que siempre es una variación de la lista:
...
A> ```
A>
A> el cual explota detalles de implementación de la JVM para evitar la
A> asignación dinámica de objetos cuando se crea un `INil`.
A>
A> Esta optimización se aplica manualmente a todas las clases con cero
A> parámetros. En realidad, Scalaz está lleno de muchas optimizaciones de esta
A> naturaleza: que son debatidas y aceptadas únicamente cuando se presentan con
A> evidencia de una mejora significativa en el rendimiento y sin riesgo de
A> sufrir cambios semánticos.
...
lenta, de sincronización de `Thread`s para asegurar una publicación segura.
...
Contraste con una *recursion*, que pone datos en un estado base y entonces
termina.
...
Las listas ligadas tienen características de rendimiento muy pobres cuando se
añaden grandes listas. Considere el trabajo que está envuelto en evaluar lo
siguiente:
...
A> Esta es una implementación simplificada: presenta un desbordamiento
A> (*overflow*) de pila que arreglaremos en el capítulo sobre Mónadas Avanzadas.
...
Las estructuras de árboles son excelentes para almacenar datos ordenados, con
cada *nodo binario* manteniendo elementos que son *menores* en una rama, y
*mayores* en la otra. Sin embargo, implementaciones ingenuas de la estructura de
datos árbol pueden *desbalancearse* dependiendo del orden de inserción. Es
posible mantener un árbol perfectamente balanceado, pero es increíblemente
ineficiente dado que cada inserción efectivamente reconstruye el árbol completo.
...
inteligencia artificial, un árbol de Rose puede ser usado en [algoritmos de
agrupamiento](https://arxiv.org/abs/1203.3468) para organizar datos en una
jerarquía de cosas cada vez más similares. Es posible representar documentos XML
con un árbol Rose.

Cuando trabajamos con datos jerárquicos, considere usar un árbol Rose en lugar
de hacer una estructura de datos a la medida.
...
A> `<++>` es el bombardero TIE. Ciertamente, enviar torpedos de protones es un
A> poco exagerado: es la misma cosa que un bombardero TIE `Monoid` `|+|`
...
A> Almacenar instancias de la typeclass en la ADT es considerado como mal estilo
A> y también incrementa el requerimiento de memoria en 64 bits por cada entrada.
A> La implementación de `FingerTree` tiene casi una década de edad y ya es
A> necesario que se reescriba.
...
A> librería estándar de nombre similar, dado que se trata, efectivamente, de un
A> constructor de colecciones.
...
A> `Cord` en Scalaz 7.2 no es, desgraciadamente, tan eficiente como debería
A> serlo. Esto se arregla en Scalaz 7.3 mediante una [estructura de datos ad hoc
A> optimizada para concatenación de
A> `String`](https://github.com/scalaz/scalaz/pull/1793).

### Cola de prioridad `Heap`

Una *cola de prioridad* es una estructura de datos que permite una rápida
inserción de elementos ordenados, permitiendo duplicados, con un rápido acceso
al valor *mínimo* (la prioridad más alta). La estructura no es necesaria para
almacenar los elementos no mínimos en orden. Una implementación ingenua de una
cola de prioridad podría ser
...
A> `Foldable.length`, a un costo de 64 bits por entrada. Una variante de `Heap`
A> podría crearse con una huella (tamaño) más pequeño, pero un `Foldable.length`
A> más lento.
...
sub-árboles de rango idéntico, colocamos el valor mínimo de manera optimista al
frente:
...
En este capítulo hemos revisado rápidamente los tipos de datos que Scalaz tiene
que ofrecer.

No es necesario recordad todo lo estudiado en este capítulo: piense que cada
sección es la semilla o corazón de una idea.

El mundo de las estructuras de datos funcionales es una área activa de
investigación. Publicaciones académicas aparecen regularmente con nuevos
enfoques a viejos problemas. La implementación de una estructura de datos
funcionales a partir de la literatura sería una buena contribución al ecosistema
de Scalaz.

<!--  LocalWords:  Scalaz polimórfica
 -->
