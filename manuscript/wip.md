...
A> and Magnolia, hanging the compiler indefinitely if there are compile errors, and
A> requires an immortal being to debug implicit resolution problems.
...
~~~~~~~~
  ...
  res: String :: Long :: HNil.type = hello :: 13 :: HNil
...
need to provide an instance for the empty `HNil.type`

{lang="text"}
~~~~~~~~
  implicit def hnil: DerivedEqual[HNil.type]
~~~~~~~~
...
But wait. It doesn't compile

{lang="text"}
~~~~~~~~
  [error] ambiguous implicit values:
  [error]  both value hnil in object DerivedEqual of type => DerivedEqual[HNil.type]
  [error]  and value cnil in object DerivedEqual of type => DerivedEqual[CNil]
  [error]  match expected type DerivedEqual[R]
  [error]   implicit val equal: Equal[Baz.type] = DerivedEqual.gen
  [error]                                                      ^
~~~~~~~~

Welcome to shapeless compilation errors!

The problem, which is not at all evident in the error, is that the compiler is unable to work out what `R` is, and gets caught thinking it is something else. We need to fix the types
