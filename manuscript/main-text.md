...
##### FIXME: try to linearise? introduce Equal / Order first

The scalaz typeclasses and data types begin to become entwined from
here. We'll see typeclasses in the type signatures that we've not
introduced yet, such as `Equal` and `Order`, but hopefully the
intention is clear.


##### FIXME: how to introduce this? It's HUGE
