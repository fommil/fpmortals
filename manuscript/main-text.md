...
4.  `strengthL` pairs the result of an `F[B]` with a constant `A` on the left.
5.  `strengthR` pairs the result of an `F[A]` with a constant `B` on the right.
6.  `lift` takes a `A => B` and returns a `F[A] => F[B]`, or in
