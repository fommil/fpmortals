...
parameter, `F`, a verbose type that represents the combination of all
the algebras inside `Deps`, roughly `Either[Drone[_], Machines[_]]`.
...
`FS[A]` methods as `FreeS[F, A]`, which is *monadic* (i.e. has an
implicit `Monad`) and can be the context of a `for` comprehension. In
the Introduction, we learnt the benefits of this abstraction pattern.
...
fold returns `FreeS[F, WorldView]`.
...
~~~~~~~~
  object Data {
    ...
    // +52 mins
    val time2 = ZonedDateTime.parse("2017-03-03T18:59:00.000+01:00[Europe/London]")
    // +59 mins
    val time3 = ZonedDateTime.parse("2017-03-03T19:06:00.000+01:00[Europe/London]")
    // +5 hours
    val time4 = ZonedDateTime.parse("2017-03-03T23:07:00.000+01:00[Europe/London]")
