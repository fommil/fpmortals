...
as importing the `object` members, or calling by full name.
...
~~~~~~~~
  ...
  import math._ ; sin(1.0)
  math.sin(1.0)
~~~~~~~~

However, it can be clunky to use `object` methods since it reads
inside-out, not a flow from left to right: it's Java's static methods
vs class methods all over again.
...
Often you might want to just skip the `object` definition and go
straight for an `implicit class`, keeping boilerplate to a minimum:
...
*typeclasses*. A typeclass is a trait that:
...
typeclasses are in the Scala standard library. This is a simplified
`scala.Numeric` to demonstrate the principle:
...
be `T`, not `Complex[T]`, and would require `T` to implement `sqrt`.

`scala.math.Numeric` tries to do too much and does not generalise
beyond real numbers. This is a good lesson that typeclasses should not
become monolithic APIs, but are better if broken into parts.
...
First, the normal variable scope is searched for implicits (`val` or
`def`), in order:
...
`implicit def`, are rather vague: the compiler implementation is the
de facto specification. However, unless you are writing a typeclass
derivation library, you are unlikely to encounter any problems.

### Libraries with Typeclass Derivation

-   circe / scalacheck-shapeless
