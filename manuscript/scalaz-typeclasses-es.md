...
Antes de introducir la jerarquía de typeclasses, echaremos un vistazo a los
cuatro métodos más importantes desde una perspectiva de control de flujo: los
métodos que usaremos más en las aplicaciones típicas de PF:
...
Este capítulo es más largo de lo usual y está repleto de información: es
perfectamente razonable abordarlo en varias sesiones de estudio. Recordar todo
requeriría poderes sobrehumanos, de modo que trate este capítulo como una manera
...
A> `|+|` es conocido como el operador TIE Fighter. Existe un operador Avanzado
A> TIE Fighter en una sección más adelante, lo cual es muy excitante.
...
A> Viktor Klang, de Lightbend, dice haber acuñado la frase [entrega
A> efectivamente una
A> vez](https://twitter.com/viktorklang/status/789036133434978304) para el
A> proceso de mensajes con operaciones idempotentes, es decir, `Band.append`.
...
Crearemos un esquema simple de plantillas para demostrar el principio, pero
tenga en mente que un sistema realista tendría un ADT más complicado.
...
A> Cuando introdujimos las typeclasses en el capítulo 4 dijimos que sólo podía
A> haber una implementación de una typeclass para un tipo de parámetro dado, es
A> decir, sólo existe un único `Monoid[Option[Boolean]]` en la aplicación. Las
A> *instancias huérfanas* tales como *lastWins* son aquellas que con más
A> facilidad rompen la coherencia.
A>
A> Podríamos intentar justificar el rompimiento de la coherencia de typeclasses
A> al hacer `lastWins` privado, pero cuando lleguemos a la typeclass `Plus`
A> veremos una mejor manera de implementar nuestro `Monoid`. Cuando lleguemos a
A> los tipos marcados (*tagged*), veremos una forma incluso mejor: usando
A> `LastOption` en lugar de `Option` en nuestro modelo de datos.
A>
...
Sin embargo, en PF preferimos el uso de typeclasses para tener funcionalidad
polimórfica e incluso el concepto de igualdad es capturado en tiempo de
compilación.
...
Las cosas que tienen un orden también podrían ser discretas, permitiéndonos
avanzar o retroceder hacia un sucesor o predecesor:
...
A> `\-->` es el sable de luz de Scalaz. Esta es la sintaxis de un programador
A> funcional. No es tan torpe o aleatoria como `fromStepToL`. Una sintaxis
A> elegante... para una era más civilizada.
...
Ahora nos enfocamos en las cosas que pueden mapearse, o recorrerse, en cierto
sentido:
...
1. `void` toma una instancia de `F[A]` y siempre devuelve un `F[Unit]`, y se
   olvida de todos los valores a la vez que preserva la estructura.
2. `fproduct` toma la misma entrada que `map` pero devuelve `F[(A, B)]`, es
   decir, devuelve el contenido dentro de una tupla, con el resultado obtenido
   al aplicar la función. Esto es útil cuando deseamos retener la entrada.
3. `fpair` repite todos los elementos de `A` en una tupla `F[(A, A)]`
4. `strengthL` empareja el contenido de una `F[B]` con una constante `A` a la
   izquierda.
5. `strenghtR` empareja el contenido de una `F[A]` con una constante `B` a la
   derecha.
6. `lift` toma una función `A => B` y devuelve una `F[A] => F[B]`. En otras
   palabras, toma una función del contenido de una `F[A]` y devuelve una función
   que opera **en** el `F[A]` directamente.
7. `mapply` nos obliga a pensar un poco. Digamos que tenemos una `F[_]` de
   funciones `A => B` y el valor `A`, entonces podemos obtener un `F[B]`. Tiene
   una firma/signatura similar a la de `pure` pero requiere que el que hace la
   llamada proporcione `F[A => B]`.
...
A> Cuando Scalaz proporciona funcionalidad adicional como sintaxis, más bien que
A> en la typeclass en sí misma, es debido a la compatibilidad binaria.
A>
A> Cuando una versión `X.Y.0` de Scalaz se publica, no es posible añadir métodos
A> a la typeclass en esa serie de publicaciones para Scala 2.10 y 2.11. Por lo
A> tanto, es de mucho valor leer tanto el código fuente de la typeclass y su
A> sintaxis.
...
Hay tantos métodos que necesitaremos revisarlos en partes, empezando con los
métodos abstractos:
...
A> `foldRight` es conceptualmente la misma operación que `foldRight` en la
A> librería estándar de Scala. Sin embargo, existe un problema con la firma de
A> la versión de la librería estándar para `foldRight`, que está resuelto en
A> Scalaz: estructuras de datos muy grandes pueden ocasionar un stack overflow.
A> `List.foldRight` hace trampa mediante implementar `foldRight` como un
A> `foldLeft` invertido.
A>
A> ```scala
...
A> ```
A>
A> pero el concepto de invertir no es universal y este truco no puede usarse
A> para todas las estructuras de datos. Por ejemplo, digamos que deseamos
A> encontrar un número pequeño en un `Stream`, con salida temprana:
A>
A> ```scala
...
A>     ...
A> ```
A> Scalaz resuelve el problema mediante tomar un parámetro *byname* para el
A> valor agregado
A>
A> ```scala
...
A>
A> Es digno de mención que no todas las operaciones son seguras para el stack en
A> `foldRight`. Si se requiriera de evaluar todos los elementos también
A> podríamos obtener un `StackOverflow` con un `EphemeralStream` de Scalaz
A>
A> ```scala
...
A> Ya hemos visto la `NonEmptyList` en capítulos anteriores. Con el propósito de
A> ser breves usaremos el type alias `Nel` en lugar de `NonEmptyList`.
A>
A> También hemos visto `IList` en capítulos anteriores, recuerde que se trata de
A> una alternativa a la `List` de la librería estándar cuyos métodos impuros,
A> como `apply`, han sido removidos.
...
Por ejemplo, digamos que tenemos una lista de palabras y que deseamos borrar las
palabras que ya hemos encontrado. El algoritmo de filtrado no permite procesar
las palabras de la lista una segunda vez de modo que pueda escalarse a un stream
infinito:
...
las cuáles deberían tener sentido a partir de las firmas/signaturas de tipo.
Ejemplos:
...
- "si me das un `JsDecoder` para un `Double`, y una manera de ir de un `Double`
  a un `Alpha`, entonces yo puedo darte un `JsDecoder` para un `Alpha`".
- "si me das un `JsEncoder` par un `Double`, y una manera de ir de un `Alpha` a
  un `Double`, entonces yo puedo darte un `JsEncoder` para un `Alpha`".
...
A> `<*>` es el operador Avanzado TIE Fighter, como lo usaría Darth Vader. Es
A> apropiado dado que parece como un padre enojado.
...
- construir algunas typeclasses para el tipo producto `C` a partir de sus
  constituyentes `A` y `B`
- ejecutar *efectos* en paralelo, como en las álgebras del drone y de google que
  creamos en el Capítulo 3, y entonces combinando sus resultados.
...
A> El operador `|@|` tiene muchos nombres. Algunos lo llaman la *sintaxis de
A> producto cartesiano*, otros lo llaman el *panecillo de canela*, el *Almirante
A> Ackbar* o el *Macaulay Culkin*. preferimos llamarlo operador *El grito*, en
A> honor a la pintura de Munch, debido a que también es el sonido que nuestro
A> CPU hace cuando está paralelizando Todas las Cosas.
...
A pesar de ser más común su uso con efectos, `Apply`funciona también con
...
Si nosotros deseamos únicamente la salida combinada como una tupla, existen
métodos para hacer sólo eso:
...
A> Todas estas clases de optimizaciones son posibles en PF, porque todos los
A> métodos son deterministas, lo que se conoce como *referencialmente
A> transparentes*.
A>
A> Si un método devuelve un valor distinto cada vez que se invoca, es *impuro* y
A> arruina el razonamiento y las optimizaciones que de otra manera podríamos
A> hacer.
A>
A> Si la `F` es un efecto, tal vez una de nuestras álgebras (drone o Google), no
A> significa que el resultado de llamar al álgebra se almacene en caché. Más
A> bien una referencia a la operación es la que se guarda en caché. La
A> optimización de rendimiento con `ifM` es notable únicamente para estructuras
A> de datos, y es más pronunciada con la dificultad del trabajo en cada
A> alternativa.
A>
A> Exploraremos el concepto de determinismo y de almacenamiento en caché en más
A> detalle en el próximo capítulo.
...
- **Identidad**: `fa <*> pure(identity) === fa`, (donde `fa` está en `F[A]`) es
  decir, aplicar `pure(identity)` no realiza ninguna operación.
- **Homomorfismo**: `pure(a) <*> pure(ab) === pure(ab(a))` (donde `ab` es una `A
  => B`), es decir aplicar una función `pure` a un valor `pure` es lo mismo que
  aplicar la función al valor y entonces usar `pure` sobre el resultado.
- **Intercambio**: `pure(a) <*> fab === fab <*> pure (f => f(a))`, (donde `fab`
  es una `F[A => B]`), es decir `pure` es la identidad por la izquierda y por la
  derecha.
...
- **Asociatividad**: `fa.bind(f).bind(g) === fa.bind(a => f(a).bind(g))` donde
  `fa` es una `F[A]`, `f` es una `A => F[B]` y `g` es una `B => F[C]`.
...
que son equivalentes para nuestra álgebra, pero no en general. Aquí se están
haciendo muchas suposiciones sobre la API de Google Container, pero es una
elección razonable que podemos hacer.
...
porque sabemos que son conmutativos entre sí. Cuando tengamos que interpretar
nuestra aplicación, más adelante en el libro, tendremos que proporcionar
evidencia de que estos efectos son de hecho conmutativos, o una implementación
asíncrona podría escoger efectuar las operaciones de manera secuencial por
seguridad.

Las sutilezas de cómo lidiar con el reordenamiento de efectos, y cuáles son
estos efectos, merece un capítulo dedicado sobre mónadas avanzadas.
...
A> `<+>` es el interceptor TIE, y a estas alturas ya casi nos quedamos sin naves
A> caza TIE...
...
`withFilter`nos permite hacer uso del soporte para `for` comprehension de Scala,
...
`msuml` realiza un `fold` utilizando el `Monoid`de `PlusEmpty[G]` y `collapse`
...
Algunas typeclasses en Scalaz existen por sí mismas y no son parte de una
jerarquía más grande.
...
y si un `Functor[F]` se proporciona entonces `zipWith`puede comportarse como
...
Una *co-cosa* típicamente tiene la firma/signatura de tipo opuesta a lo que sea
que una *cosa* hace, pero no necesariamente a la inversa. Para enfatizar la
relación entre una *cosa* y una *co-cosa*, incluiremos la firma/signatura de la
*cosa* siempre que sea posible.
...
Una aplicación de zipper es para *automatas celulares*, que calculan el valor de
cada celda en la siguiente generación mediante realizar un cómputo basándose en
la vecindad de dicha celda.
...
!Esto fue bastante material! Hemos apenas explorado la librería estándar de
funcionalidad polimórfica. Pero para poner el asunto en perspectiva: hay más
traits en la API de collecciones de la librería estándar de Scala que
typeclasses en Scalaz.

Es normal que una aplicación de PF usar un porcentaje pequeño de la jerarquía de
tipos, con la mayoría de su funcionalidad viniendo de álgebras particulares del
dominio y de typeclasses. Inclusive si las typeclasses del dominio específico
son simples clones especializados de algo que ya existe en Scalaz, está bien
refactorizarlo después.

Para ayudar al lector, hemos incluído un sumario de las typeclasses y sus
métodos primarios en el apéndice, tomando inspiración del sumario cuyo autor es
Adam Rosien's: [Scalaz
Cheatsheet](http://arosien.github.io/scalaz-cheatsheets/typeclasses.pdf).

Para ayudarle, Valentin Kasas explica cómo [combinar
`N`cosas](https://twitter.com/ValentinKasas/status/879414703340081156):
